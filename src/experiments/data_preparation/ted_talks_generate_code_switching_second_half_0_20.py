from multiprocessing import freeze_support
from pathlib import Path

import pandas as pd

from core.synthetic_data.generate_code_switching import (
    generate_code_switching_last_n_words,
    postprocess_source_code,
)
from experiments.data_preparation.wikipedia_generate_code_switching_single_word_40_50 import language_index_to_code

if __name__ == "__main__":
    freeze_support()

    SUBSET_SIZE = 1000

    df_path = Path(__file__).parent.joinpath("../../../data/ted_talks.jsonl")
    df = pd.read_json(df_path, lines=True)

    out_path = Path(__file__).parent.joinpath("../../../data/ted_talks_code_switching_second_half.jsonl")
    postprocess_source_code(df, df_path, seq_len=20)
    translate_codes = [language_code for language_code in language_index_to_code if language_code != "eng_Latn"]
    for language_code in translate_codes:
        print(f"Translating to {language_code}...\n\n")
        generate_code_switching_last_n_words(df, language_code, out_path, n="half")
